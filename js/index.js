$(document).ready(function () {
  ...
  faqPanels.each((_index, panel) =>
    panel.addEventListener("click", accordionHandler)
  );
    ...
    fetch(request)
      .then((response) => {
        ...
        if (response.status === 200) {
          ...
          $(this)
            .find(".modalFormErrorFeedback")
            .text(
              "An error occur while sending your message, please try again."
            );
      ...
      .then((response) => {
        ...
        if (response.status === 200) {
          ...
          $(this)
            .find(".modalFormErrorFeedback")
            .text(
              "An error occur while creating your initiative, please try again."
            );
          $(this).find(".modalFormErrorFeedback").css("display", "block");
        }
      })
  ...
  function handleNewsletterForm(event) {
    ...
    fetch(request)
      .then((response) => {
        if (response.status === 200) {
          $(this)
            .find(".subscriptionFeedback")
            .text("Successful! Thank you for subscribing.");
          $(this).find(".subscriptionFeedback").addClass("success");
          $(this).find(".subscribeButton").prop("disabled", false);
          $(this).trigger("reset");
          setTimeout(
            () => $(this).find(".subscriptionFeedback").removeClass("success"),
            2000
          );
        } else {
          $(this)
            .find(".subscriptionFeedback")
            .text("An error occur while subscribing, please try again.");
